...
void internalForward(const arma::mat& transition, const arma::cube& emission, const arma::vec& init,
  const arma::ucube& obs, arma::cube& alpha, arma::cube& emission_probs, arma::mat& scales, unsigned int threads) {

#pragma omp parallel for if(obs.n_slices >= threads) schedule(static) num_threads(threads) \
  default(none) shared(alpha, emission_probs, scales, obs, init, emission, transition)
    for (unsigned int k = 0; k < obs.n_slices; k++) {
      alpha.slice(k).col(0) = init;
      emission_probs.slice(k).col(0).ones();
      for (unsigned int r = 0; r < obs.n_rows; r++) {
        alpha.slice(k).col(0) %= emission.slice(r).col(obs(r, 0, k));
        emission_probs.slice(k).col(0) %= emission.slice(r).col(obs(r, 0, k));
      }
      scales(0, k) = 1.0 / sum(emission_probs.slice(k).col(0));
      emission_probs.slice(k).col(0) *= scales(0, k);
      ...
      for (unsigned int t = 1; t < obs.n_cols; t++) {
        ...
        emission_probs.slice(k).col(t).ones();
        for (unsigned int r = 0; r < obs.n_rows; r++) {
...
void internalForwardx(const arma::sp_mat& transition_t, const arma::cube& emission,
  const arma::mat& init, const arma::ucube& obs, arma::cube& alpha, arma::cube& emission_probs, arma::mat& scales,
  unsigned int threads) {

#pragma omp parallel for if(obs.n_slices >= threads) schedule(static) num_threads(threads) \
  default(none) shared(alpha, emission_probs, scales, obs, init, emission, transition_t)
    for (unsigned int k = 0; k < obs.n_slices; k++) {
